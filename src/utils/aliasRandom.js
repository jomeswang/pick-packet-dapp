...
(function (root, VoseAliasMethod) {
  ...
  (function () {
     ...
     */
    const scaleForAliasMethod = function (probabilities) {
      const total = probabilities.reduce((a, b) => a + b);

      if (total > 1) {
        // Divide every element by the total
        probabilities = probabilities.map(val => val / total);
      } else if (total < 1) {
    ...
    VoseAliasMethod.prototype.init = function (probabilities) {
      let average, small, large, i, less, more;
      ...
      for (i = 0; i < this.probabilityLength; i++) {
        if (
          isNaN(parseFloat(probabilities[i])) ||
          !isFinite(probabilities[i])
        ) {
          throw new Error(
            'All elements in probabilities array must be numeric',
          );
        }
      }
